...
class App:
    ...
    def listar_relatorios(self):
        ...
        self.relatorios_lista = pr.mapa_arquivos("",pr.reports_path)
        self.lista_relatorios.delete(0,END)
        for key in self.relatorios_lista:
            self.lista_relatorios.insert(END,key)

    def listar_relatorios_dia(self):
        pr.mudar_diretorio(pr.reports_path)
        filtro = pr.data_hora().split("_")[0]
        self.relatorios_lista = pr.mapa_arquivos(filtro,pr.reports_path)
        self.lista_relatorios.delete(0,END)
        for key in self.relatorios_lista:
            self.lista_relatorios.insert(END,key)

        print("Filtro: ",filtro)
        print("Relatorios de hoje:",self.relatorios_lista)
    ...
    def adicionar_produto(self):
        ...
        if (type(quantidade) != str) and (type(preco) != str) and ((type(estoque)!=str) or estoque == "n/a"):
            ...
            self.detalhes_pedido.insert(
                "end",
                f'{nome} | {quantidade} x {unidade} | R${preco} | Total: R${self.total_produto}\n')
    ...
    def abrir_gerenciar_estoque(self):
        if not self.estoque_open:
            ...
            self.janela_gerenciar_estoque.title("Manejo de Estoque")
            ...
            self.botao_filtrar = Button(self.frame_l,text="Filtrar",font="arial 25")
            self.lista = Listbox(
                self.frame_l,
                font="arial 25"
                )
